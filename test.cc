...
int main (){
  ...
  string str = "Experience with application programming and languages ( e.g Java , J2EE , VB , VBA , ASP , .Net , C , C# , COBOL4 , COBOL , XML , SQL , PL-SQL ) . ";
  ...
  for (unordered_map<string, unordered_map<string, string> >::iterator iter = keywords.begin(); iter != keywords.end(); ++iter){
      ...
      if (m.str(1) != ""){
        ...
        ostringstream oss;
        oss << char(127) << "_" << i;
        string replace_keyword = oss.str();
        history[i++] = keyword;

        str.replace(start_index, end_index - start_index + 1, replace_keyword);
      }
  }
